	...
	const addOrSwitchNetwork = () => {
		window?.ethereum
			.request({
				...
				params:
					chainIdDeployEnvMap["local"]['params'],
			})
		...
		{account && isOnCorrectNetwork ?
			<div>
				...
				>
					...
					<span className='py'>{`${account.slice(0, 5)}...${account.slice(-3)}`}</span>
				</button>
			...
			isOnCorrectNetwork ?
				<div>
					<button
						...
						className="flex items-center font-mont whitespace-nowrap h-12 rounded-lg border border-inactive-accent bg-inactive-accent-inside py-2 px-6 text-black font-semibold cursor-pointer hover:border-active-accent"
						disabled={isConnecting}
					>
						{'Connect Wallet'}
					</button>
				...
				<button onClick={addOrSwitchNetwork}
					className="flex items-center font-mont whitespace-nowrap h-12 rounded-lg border border-inactive-accent bg-inactive-accent-inside py-2.5 px-6 text-black font-semibold"
				>
					Use{' '}
					{
						chainIdDeployEnvMap["local"][
							'networkName'
						]
					}{' '}
					Network
